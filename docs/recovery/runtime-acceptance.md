...
| Templater (`templater-obsidian`) | Set template folder to `Templates/Templater`; leave automatic new-file/folder triggers and system commands off. `Person` renders a title; `Delayed Person` waits eight seconds; `Renamed Person` appends ` Renamed` to the created filename; `Failure` throws intentionally. Use these templates for creation, including Delayed Person for pending-action guard checks. Entities insertion is explicitly unavailable: selecting Person/Delayed Person in an insertion provider must run no engine and leave text unchanged with a manual alternative. Separately verify the tester-initiated Templater: Open insert template modal / existing hotkey route; it is outside Entities guards. Record partial files left by failures. [Templater file functions](https://silentvoid13.github.io/Templater/internal-functions/internal-modules/file-module.html) document title/rename behavior. |
...
[creation service](../../src/entityCreation.ts), [action coordinator](../../src/actionCoordinator.ts), [modal](../../src/userComponents.ts).
...
| A7: pending action (R4) | Use Delayed Person's eight-second wait. Separately edit inside/outside the trigger, switch away/back, replace text, reuse editor for another file, delete source, close popout, disable plugin. Repeat with Template actionType=insert: type `/Delayed` and select Delayed Person (slash queries stop at spaces); inspect both documents. CURRENT insertion validates neither revision nor lifecycle; Templater also mutates the active editor itself. | Use delayed **creation** to test Entities guards: no stale Entities write, one transaction when valid, and useful cancellation/failure feedback. Delivered edit→undo and observed switch-away/back invalidate; undelivered same-task programmatic activation is an explicit limit. A successfully created file remains discoverable when insertion is cancelled. **Insertion rows must visibly say unavailable before selection and perform zero engine/command/prompt/creation work and zero text/selection changes**; feedback includes the template path and manual Templater command/hotkey route. Repeated selection settles once. Verify actual integration mutations separately; already-running trusted native effects are outside the guard. No unhandled rejection. |
